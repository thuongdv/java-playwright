...
/**
 * Loads test configuration from a .properties file. System properties (set via Maven -D flags)
 * always override file values.
 *
 * <p>Resolution order (highest → lowest priority):
 *
 * <ol>
 *   <li>JVM system property (-Dbrowser=edge)
 *   <li>Properties file (config/default.properties or -Dconfig.file=...)
 *   <li>Hard-coded default (fallback inside {@link #get(String, String)})
 * </ol>
 */
public class ConfigManager {
  ...
  /** Returns the int value for {@code key}, giving priority to a JVM system property. */
  ...
  public int getDefaultTimeout() {
    return getInt("timeout.default", 30_000);
  }

  public int getNavigationTimeout() {
    return getInt("timeout.navigation", 60_000);
  }

  public int getViewportWidth() {
    return getInt("viewport.width", 1280);
  }

  public int getViewportHeight() {
    return getInt("viewport.height", 720);
  }
  ...
  public String getTraceMode() {
    return get("trace.mode", "retain-on-failure");
  }

  public String getOutputDir() {
    return get("output.dir", "target/test-output");
  }
  ...
  private void loadFromFile(String filePath) {
    ...
    if (is == null) {
      try {
        ...
        log.warn(
            "Config file '{}' not found on classpath or filesystem — using defaults / system properties only.",
            filePath);
        return;
      }
    }
